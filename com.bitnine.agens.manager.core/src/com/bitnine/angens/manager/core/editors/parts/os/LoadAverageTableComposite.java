 ...
 *
 */
public class LoadAverageTableComposite extends AgensTimeseriesChartComposite {
	 ...
	 * @param instance
	 */
	public LoadAverageTableComposite(Composite parent, UserDBDAO userDB, Instance instance) {
	...
	public List<?> getUIData() throws Exception {
		List<Map> listLine = AgensManagerSQLImpl.getSQLMapQueryInfo(userDB, "load_average", getRangeSnapId());
		if (listLine.isEmpty()) return new ArrayList<>();
		
		TimeDataItem2D[] min1 = new TimeDataItem2D[listLine.size()];
		TimeDataItem2D[] min5 = new TimeDataItem2D[listLine.size()];
		TimeDataItem2D[] min15 = new TimeDataItem2D[listLine.size()];
		
		for (int i=0; i<listLine.size(); i++) {
			Map mapData = listLine.get(i);
			
			String strTime = String.format("new Date(\"%s\")", ""+mapData.get("replace"));
			min1[i] 	= new TimeDataItem2D(strTime, ((BigDecimal) mapData.get("1min")).doubleValue());
			min5[i] 	= new TimeDataItem2D(strTime, ((BigDecimal) mapData.get("5min")).doubleValue());
			min15[i] 	= new TimeDataItem2D(strTime, ((BigDecimal) mapData.get("15min")).doubleValue());
		}
		
		List<TimeDataGroup> listDataGroup = new ArrayList<>();
		listDataGroup.add(new TimeDataGroup(min1, "1min",  ColorsSWTUtils.CAT10_COLORS[0]));
		listDataGroup.add(new TimeDataGroup(min5, "5min", ColorsSWTUtils.CAT10_COLORS[1]));
		listDataGroup.add(new TimeDataGroup(min15, "15min",  ColorsSWTUtils.CAT10_COLORS[2]));
