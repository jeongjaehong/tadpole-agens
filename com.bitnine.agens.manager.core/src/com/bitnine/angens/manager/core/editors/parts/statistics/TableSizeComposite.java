 ...
 *
 */
public class TableSizeComposite extends AgensChartComposite {
	private static final Logger logger = Logger.getLogger(TableSizeComposite.class);
	private PieChart pieTableSize;
	 ...
	 * @param instance
	 */
	public TableSizeComposite(Composite parent, UserDBDAO userDB, Instance instance) {
		...
		grpTransactionStatistics.setText("Table Size");

		pieTableSize = new PieChart(grpTransactionStatistics, SWT.NONE);
		...
		pieTableSize.setLayout(gl_grpConnectionInfo);
		...
		try {
			...
			pieTableSize.setItems(listDataItem.toArray(new DataItem[listDataItem.size()]));
		} catch(Exception e) {
	...
	public List<?> getUIData() throws Exception {
		...
		try {
			List<Map> listLine = AgensManagerSQLImpl.getSQLMapQueryInfo(getUserDB(), "table_size", getLastSnapId());
			int intColumnPos = 0;
			for (Map mapTableSize : listLine) {
				String strTitle = ""+mapTableSize.get("Title");
				int intMB = Integer.parseInt(""+mapTableSize.get("MiB"));
	...
	public void refreshUI(final List<?> listData) {
		try {
			final Display display = pieTableSize.getDisplay();
		    display.asyncExec( new Runnable() {
		    	public void run() {
		    		pieTableSize.setItems(listData.toArray(new DataItem[listData.size()]));
		    	}
		    } );
